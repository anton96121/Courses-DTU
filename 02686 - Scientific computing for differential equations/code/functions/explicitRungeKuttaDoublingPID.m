function [x,t,function_calls,hs,rs] = explicitRungeKuttaDoublingPID(f,param,h,t0,T,x0,A,b,c,Atol,Rtol,hmin,hmax,eps_tol,p,initial_step_algo)
    N = ceil((T-t0)/hmin);
    s = length(b);
    d = length(x0);
    t = zeros(1,N+1);
       ...
       while(~accept_step)
           ...
           if e<=1
               ...
               elseif i == 3
                   ...
                   h = h*max(hmin,min(hmax,fac*(eps_tol/e)^(0.58/(p+1))*(e1/e)^(0.21/(p+1))*(e2/e1)^(0.1/(p+1)))); % eq 4.13 in Harier
                   e1 = max(e,10^(-10));
                   e2 = e1;
               end
