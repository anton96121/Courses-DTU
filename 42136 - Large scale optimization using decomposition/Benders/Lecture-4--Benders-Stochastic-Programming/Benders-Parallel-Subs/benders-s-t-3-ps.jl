...
function solve_master(alphabar, betabar)
    ...
    @constraint(masterproblem,
                ...
                +sum(betabar[w][j]*demandScenario[j,w] for j=1:numCustomers) for w=1:numScenarios) >= q)
...
function solve_subproblem(xbar, w)
    ...
    @variable(subproblem, alpha[j=1:numCustomers])
    @variable(subproblem, beta[j=1:numCustomers]>=0)

    @objective(subproblem, Min, sum(alpha[j]*sum(xbar[i,j] for i=1:numFactories) for j=1:numCustomers)
                                +sum(beta[j]*demandScenario[j,w] for j=1:numCustomers))

    @constraint(subproblem, [j=1:numCustomers], alpha[j]+beta[j] == salePrice*pi[w])
    @constraint(subproblem, [j=1:numCustomers], alpha[j] >= -wasteCost*pi[w])
...
let
    ...
    it=1
    while (UB-LB>Delta)
        #scenarios are solved in sequence
