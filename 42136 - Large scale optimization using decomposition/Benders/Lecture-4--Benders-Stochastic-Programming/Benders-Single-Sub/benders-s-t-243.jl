...
function solve_master(alphabar, betabar)
    ...
    @constraint(masterproblem,
                ...
                +sum(betabar[j,w]*fullDemandScenarios[w][j] for j=1:numCustomers for w=1:numScenarios) >= q)
...
function solve_subproblem(xbar)
    ...
    @objective(subproblem, Min, sum(sum(alpha[j,w]*(xbar[i,j]) for i=1:numFactories) for j=1:numCustomers for w=1:numScenarios)
                                +sum(beta[j,w]*fullDemandScenarios[w][j] for j=1:numCustomers for w=1:numScenarios))

    @constraint(subproblem, [j=1:numCustomers, w=1:numScenarios], alpha[j,w]+beta[j,w] >= salePrice*prob[w])
    @constraint(subproblem, [j=1:numCustomers, w=1:numScenarios], alpha[j,w] >= -wasteCost*prob[w])
